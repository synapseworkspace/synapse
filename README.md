...
- `GET /v1/adoption/synthesis-graph/diagnostics` now combines those two diagnostics surfaces into one graph snapshot: unified nodes/edges across tools, sources, agents, capabilities, processes, tools, decisions, plus roll-up summaries for both `tooling-map` and `data-sources`. This is the first general-purpose adoption graph view rather than a page-family-specific debug endpoint.
