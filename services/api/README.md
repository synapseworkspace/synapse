...
- `POST /v1/adoption/first-run/bootstrap` (create starter wiki pages on first run: `Agent Profile`, `Data Map`, `Runbook`, `Decisions Log`, optional Step-0 `state` sync)
...
- `POST /v1/adoption/agent-wiki-bootstrap` (preview/apply “Agent Wiki Bootstrap” out-of-box pack with `preview_apply_flow`, `quality_report`, optional `bootstrap_publish_core`, include-flags for `tooling_map`, `process_playbooks`, `company_operating_context`, `state_snapshot`, and schema-contract seeded pages)
