...
- 2026-05-03: Hardened the new shared-memory push path with delivery audit + retry: `shared_memory_fanout_deliveries` now records dry-run/planned and live hook deliveries, `GET /v1/agents/shared-memory/fanout-deliveries` exposes recent delivery history, `POST /v1/agents/shared-memory/fanout-deliveries/{delivery_id}/retry` can re-dispatch a failed or past delivery, and shared-memory health/UI now flag recent hook failures so push-mode deployments are observable instead of opaque.
- 2026-05-03: Added optional push-style shared-memory delivery: new `shared_memory_fanout_hooks` storage plus `POST/GET /v1/agents/shared-memory/fanout-hooks` and `POST /v1/agents/shared-memory/fanout-dispatch` let operators register runtime callbacks and dry-run or execute invalidation / impact / publish-preview fanout payloads; health metrics and Operations UI now also show whether the current space still relies only on polling.
- 2026-05-03: Added the first long-lived private/team shared-memory corpus on top of draft overlays: new Postgres-backed `shared_memory_entries` storage, `POST /v1/agents/shared-memory/entries` and `GET /v1/agents/shared-memory/entries` APIs, SDK helpers in Python/TypeScript, and automatic inclusion of materialized entries inside shared-memory hydration / invalidation / health so runtimes can now write durable private/team notes that show up alongside published/reviewed wiki deltas.
- 2026-05-03: Moved the shared-memory track into operator-visible territory: Synapse now exposes `POST /v1/agents/shared-memory/impact` (recent change impact by agent/role/team with relevance-scored affected rows) and `GET /v1/agents/shared-memory/health` (snapshot freshness / lag / roster coverage metrics), added Python/TypeScript SDK helpers for both, and surfaced a compact `Shared memory freshness` panel in Operations so teams can see which agents are most likely behind on fresh published knowledge without digging through raw API responses.
- 2026-05-03: Advanced the shared-memory contract from implicit review heuristics toward explicit tiers: agent hydration / invalidation / impact APIs now accept `memory_tier_mode` (`published_org`, `reviewed_team`, `draft_private`), return explicit tier scope metadata, and degrade `draft_private` honestly until a true private-memory corpus is materialized; Synapse also now exposes `POST /v1/agents/shared-memory/publish-impact-preview` so operators/runtimes can see which agents a planned wiki publish is likely to affect before the change goes live.
- 2026-05-03: Lifted `draft_private` beyond a placeholder tier: trusted-role shared-memory hydration/feed paths now pull real draft-backed deltas from `wiki_draft_changes` (`pending_review` / `blocked_conflict`) alongside published/reviewed knowledge, invalidation/health metrics surface visible draft-item counts and latest draft timestamps, and the tier contract remains explicit that this is draft-backed private change memory rather than a fully separate long-lived private corpus.
- 2026-05-03: Added the next shared-memory explainability layer: shared-memory feed items now emit typed `delta_objects` (rule/playbook/source/decision/escalation/etc.) with importance and action hints, impact/preview APIs now return compact `fanout_plan` summaries (top teams/roles + refresh-now vs next-task buckets), and Operations UI shows a page-scoped publish impact preview so operators can see who a planned page publish is likely to nudge before it goes live.
- 2026-05-03: Started the next generalization layer after the logistics tooling-map correctness loop stabilized: `GET /v1/adoption/tooling-map/diagnostics` now emits typed synthesized field entries (`value`, `origin`, `confidence`) for capability/process/source/guardrail labels, exposes a reusable relation graph (`tool -> capability/process/source/guardrail` edges), and reports generic quality metrics (`field_origin_counts`, `process_equals_capability_rows`, `overlong_source_rows`) instead of only a page-level summary. This is the first production slice of a more general typed synthesis model that can be reused across domains and future compilers, not just the current logistics pilot.
- 2026-05-03: Extended the typed synthesis-debugging model to source pages as well: `GET /v1/adoption/data-sources/diagnostics` now emits typed synthesized field entries plus relation edges (`source -> agent/capability/process/tool/decision`) and generic quality metrics (`rows_with_agents`, `rows_with_capability_or_process`, `inferred_usage_rows`, `bundle_backed_rows`, `field_origin_counts`). This gives Synapse one shared provenance/confidence contract for both `Tooling Map` and `Data Sources Catalog`, which is a stronger base for future multi-domain compilers than more one-off logistics heuristics.
- 2026-05-03: Extended that typed diagnostics model to `Agent Capability Profile` as well: `GET /v1/adoption/agent-capability/diagnostics` now emits typed synthesized fields for capability/tool/source/action/task/decision structure, agent-centric relation edges (`agent -> capability/tool/source/action/task/standing_order/decision`), and generic quality metrics (`rows_with_tools`, `rows_with_sources`, `rows_with_running_instances`, `rows_with_reflection`, `contract_backed_rows`, `field_origin_counts`). This gives Synapse the same provenance/confidence model across tools, sources, and agents instead of only page-family-specific debugging.
- 2026-05-03: Extended the same typed diagnostics model to `Process Playbooks`: `GET /v1/adoption/process-playbooks/diagnostics` now emits structured playbook rows (purpose/trigger/action/owners/tools/artifacts/decision refs) with typed synthesized fields, relation edges (`playbook -> agent/tool/artifact/decision/input`), and generic quality metrics (`rows_with_owners`, `rows_with_tools`, `rows_with_artifacts`, `rows_with_decisions`, `rows_with_human_loop`, `field_origin_counts`). That means the four main compiler families now share one provenance/confidence/debugging model instead of requiring separate ad-hoc review loops.
- 2026-05-03: Added the next synthesis-observability layer above those typed diagnostics: `GET /v1/adoption/synthesis-graph/diagnostics` now emits one combined adoption graph snapshot across tool rows and source rows, with unified node/edge counts by kind/origin plus roll-up summaries for both diagnostics families. This starts turning the new typed provenance model into a reusable graph/debug substrate instead of two isolated endpoints.
- 2026-05-03: Expanded the pack architecture beyond logistics: `support_center` and `sales_revenue_ops` business profiles now resolve to dedicated `support_ops` / `sales_ops` synthesis packs instead of the neutral fallback, task semantics are no longer implicitly forced through `logistics_ops`, and shared source/process/company synthesis now honors those packs as well. That means support escalations, queue-routing SOPs, qualification/stage/handoff workflows, and revenue-ops operating context can evolve through pack policy rather than more domain branches in `main.py`.
- 2026-05-03: Continued the `core engine + domain packs` extraction into capability/tooling synthesis: `compliance_program` now resolves to dedicated `compliance_ops`, and pack hooks now provide domain-aware capability-signal bullets, sparse capability hints, tooling-map governance guidance, and empty-state wording. This keeps `Agent Capability Profile` / `Tooling Map` adaptable for logistics, support, sales, and compliance without re-embedding those business phrases into compiler core code.
- 2026-05-03: Added a follow-up synthesis correctness pass for the real logistics runtime-surface pilot: runtime-surface agent profiles now persist inferred domain/space hints so task runbooks can pick the right synthesis pack, logistics process refinement no longer leaks `driver economy` purpose into unrelated SOPs via evidence/artifact tokens, and `Tooling Map` / `Data Sources Catalog` now prefer explicit structured contract matches before falling back to broad per-agent capability/source assumptions.
- 2026-05-03: Tightened the next layer of runtime-surface synthesis quality for the logistics pilot: shared source/process labels are now normalized before rendering (`standing_order.*` -> cleaner process summaries, raw `*_latest` source/tool tokens -> shorter labels), `Tooling Map` no longer sprays the same agent-level capability/source fallback across every tool row when explicit contract matches are missing, and `fleet sync` now gets dedicated logistics SOP semantics instead of collapsing into a generic recurring workflow.
- 2026-05-03: Pushed the next `core engine + domain packs` extraction into shared content synthesis: recurring `Process Playbooks` now run through pack-level refinement hooks after the generic compiler assembles a candidate SOP, and `Company Operating Context` now gets pack-supplied snapshot notes, workflow rollups, and operating-principle framing. This keeps `generic_ops` neutral while letting `logistics_ops` make shared runbooks/context pages denser and more domain-specific without leaking that wording back into the compiler core.
...
- 2026-05-03: Moved starter/template pack definitions behind the same policy boundary: first-run starter pages, default wiki spaces per starter profile, and role-template catalogs/pages now come from `synthesis_packs` rather than a pile of `if profile == ...` / `if template_key == ...` branches in `main.py`. Behavior is unchanged, but the remaining domain-specific bootstrap surface is now mostly data/policy rather than compiler code.
- 2026-05-03: Added first-class business deployment profiles on top of packs: Synapse now exposes `/v1/adoption/business-profiles` with ready-made configs for `generic_service_ops`, `logistics_operator`, `support_center`, `sales_revenue_ops`, `compliance_program`, and `ai_employee_org`, and both `first-run/bootstrap` and `sync-presets/execute` can resolve those profiles into effective starter/template/space/bundle-promotion defaults. This gives adopters a practical “pick the nearest business shape” layer above the lower-level pack and starter knobs.
...
- 2026-05-03: Completed first operator-facing synthesis observability pass in `apps/web`: the Operations workspace now includes a compact `Synthesis observability` panel backed by `GET /v1/adoption/synthesis-graph/diagnostics`, surfacing graph size, per-family coverage cards (tooling/sources/agents/playbooks), and top synthesis concerns without forcing operators to hop across raw diagnostics endpoints.
- 2026-05-03: Hardened self-host delivery observability for the new synthesis panel: web now reports route-aware feature flags and a visible build stamp into `GET /v1/adoption/selfhost/consistency`, and the API warns explicitly when an `Operations` self-host deployment is missing `synthesis_observability_panel` support because the frontend bundle is stale.
- 2026-05-03: Started shared-memory runtime track: added roadmap doc `docs/shared-memory-roadmap.md`, introduced `GET /v1/wiki/change-feed` for machine-readable published/reviewed wiki deltas, and shipped `POST /v1/agents/shared-memory/hydrate` so runtimes can fetch one conservative task-start context bundle (`state snapshot + onboarding pack + fresh wiki changes + retrieval hints`) instead of hand-assembling shared memory from multiple surfaces.
- 2026-05-03: Advanced shared-memory runtime track into freshness phase: published page create/update paths now best-effort refresh the space `Wiki State Snapshot`, and runtimes can cheaply poll `POST /v1/agents/shared-memory/invalidation` for a scoped invalidation token before deciding whether to re-run full hydration.
- 2026-05-03: Added smarter shared-memory relevance layer: task-start hydration and invalidation now resolve `review_policy_mode=auto|published_only|reviewed_plus_published`, can automatically widen to `reviewed` for trusted roles, classify wiki changes into delta kinds (`policy/process/source/agent_profile/...`), and attach lightweight relevance scoring/reasons so runtimes can prioritize only the changes that matter to a given agent role.
- 2026-05-03: Added shared-memory runtime acknowledgement contract: fanout deliveries now carry per-delivery correlation metadata, runtimes can acknowledge `accepted/refreshed/ignored/failed` via `POST /v1/agents/shared-memory/fanout-acks`, health surfaces now track delivered-vs-acked freshness (`fanout_acks_recent`, `fanout_unacked_recent`, `fanout_ack_lag_minutes`), and the Operations panel now warns when push delivery is succeeding but no runtime acknowledgements are coming back.
- 2026-05-03: Added queued shared-memory fanout foundation: `POST /v1/agents/shared-memory/fanout-dispatch` now supports `enqueue_only=true`, pending deliveries gain queue scheduling fields, `POST /v1/agents/shared-memory/fanout-deliveries/process-pending` executes due queue items, and shared-memory health/Operations now surface `fanout_pending_queue` alongside retries/acks so operators can distinguish “queued”, “sent”, and “acknowledged”.
- 2026-05-03: Added richer lifecycle semantics for materialized private/team memory: `shared_memory_entries` now support `superseded/resolved/expired/archived` states plus lifecycle metadata (`superseded_by_entry_id`, `resolved_at`, `expires_at`, `lifecycle_reason`), SDK list/upsert contracts can filter and write these states, and shared-memory health now distinguishes active memory from lifecycle-complete entries instead of treating all long-lived notes as forever-active context.
- 2026-05-03: Added shared-memory entry lifecycle processing: `POST /v1/agents/shared-memory/entries/process-lifecycle` now finds due `expires_at` entries and can expire them in batch, SDKs expose the processor, and shared-memory health/Operations now surface `materialized_entries_due_expire` and `materialized_entries_expiring_soon` so operators can see when private/team memory is aging out but not yet processed.
- 2026-05-03: Wired shared-memory maintenance into the worker loop: new script `services/worker/scripts/run_shared_memory_maintenance.py` processes queued fanout deliveries, due retries, and expiring lifecycle entries across discovered projects, with self-host env defaults and worker-loop integration so shared memory now has a default background maintenance cadence instead of relying on manual API invocations.
- 2026-05-03: Hardened shared-memory maintenance for multi-worker self-host mode: `run_shared_memory_maintenance.py` now acquires per-project advisory locks before touching queue/retry/lifecycle processors, and worker tests cover deterministic lock-key generation so parallel maintenance loops are less likely to double-process the same shared-memory workload.
